...
class MonoDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, data_path, filenames, correspondences, height, width, frame_idxs,
                 num_scales, is_train=False, img_ext='.jpg', load_depth=False, load_pose=False):
    ...
    def __getitem__(self, index):
        ...
        for idx, fid in [[0, -1], [1, 1]]:
            ...
            # for key, val in self.correspondences[flip_str][index][idx].items():
            #     inputs[(key, source_fid, fid, flip_str)] = val
        ...
        for i in self.frame_idxs:
            ...
            if self.load_depth:
                ...
                inputs[('depth', i, 0)] = torch.tensor(depth)
                
            if self.load_pose:
                inputs[('pose', i, 0)] = self.get_pose(folder, line[2+i], do_flip)
...
class SimcolDatasetCorres(GastroDataset):
    def __init__(self, *args, **kwargs):
        super(SimcolDatasetCorres, self).__init__(*args, **kwargs)
...
class C3VDDatasetCorres(GastroDataset):
    def __init__(self, *args, **kwargs):
        super(C3VDDatasetCorres, self).__init__(*args, **kwargs)
    ...
    def get_depth(self, folder, frame_index_str, do_flip):
        depth = pil.open(self.get_depth_path(folder, frame_index_str))
    ...
    def __getitem__(self, index):
        ...
        for i in self.frame_idxs:
            ...
            if self.load_depth:
                depth = self.get_depth(folder, line[2+i], do_flip).resize((self.width, self.height))
                depth = np.array(depth) / (2**16 - 1)
                inputs[('depth', i, 0)] = torch.tensor(depth)[None, ...]
