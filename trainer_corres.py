...
class Trainer:
    def __init__(self, options):
        ...
        if self.opt.model == "monodepth2":
            ...
            self.models["depth"] = networks.DepthDecoderV2(self.models["encoder"].num_ch_enc,
                                                     self.opt.scales)
        ...
        if self.opt.model == "monodepth2":
            # self.model_optimizer = optim.Adam(self.parameters_to_train, self.opt.lr[0])
        ...
        if self.use_pose_net:
            if self.opt.model == "monodepth2":
                # self.model_pose_optimizer = optim.Adam(self.parameters_to_train_pose, self.opt.lr[3])
        ...
        # data
        datasets_dict = {"yifanwang": datasets.GastroDataset,
                         "my_kitti": datasets.GastroDataset,
                         "simcolcorres": datasets.SimcolDatasetCorres,
                         "simcol": datasets.SimcolDataset,
                         "c3vdcorres": datasets.C3VDDatasetCorres,
                         "c3vd": datasets.C3VDDataset,
                         "kitticorres": datasets.KITTIRAWDatasetCorres,
        ...
        try:
            ...
            matcher_result_load = np.load(self.opt.matcher_result, allow_pickle=True).all()

        num_train_samples = len(train_filenames)
        self.num_total_steps = num_train_samples // self.opt.batch_size * self.opt.num_epochs

        train_dataset = self.dataset(
            self.opt.data_path, train_filenames, matcher_result_load, self.opt.height, self.opt.width,
            self.opt.frame_ids, 4, is_train=True, img_ext=img_ext)
        self.train_loader = DataLoader(
            train_dataset, self.opt.batch_size, True, collate_fn=collate_fn,
            num_workers=self.opt.num_workers, pin_memory=True, drop_last=True)
        if "corres" in self.opt.dataset:
            val_dataset_name = datasets_dict[self.opt.dataset[:-6]]
        else:
            val_dataset_name = datasets_dict[self.opt.dataset]
    ...
    def train(self):
        ...
        for self.epoch in range(self.opt.num_epochs):
            if self.opt.half_epoch_matcher:
                if self.epoch > self.opt.num_epochs / 2 and not self.opt.disable_matcher:
                    self.opt.disable_matcher = True
                    print("disable matcher loss at epoch {}".format(self.epoch))
            self.run_epoch()
            if (self.epoch + 1) % self.opt.save_frequency == 0 or (self.epoch + 1) == self.opt.num_epochs:
                self.save_model()
    ...
    def compute_losses(self, inputs, outputs):
        ...
        # # calculate matcher loss
        # if not self.opt.disable_matcher:
        #     matcher_loss = 0
        #     for frame_id in self.opt.frame_ids[1:]:
        #         correspondences = self.matcher(inputs[('color', 0, source_scale)], inputs[('color', frame_id, source_scale)])
        #         for scale in self.opt.scales:
        #             matcher_loss += compute_matcher_errors(correspondences, outputs[("sample", frame_id, 0)], self.opt.width, self.opt.height, self.opt.batch_size)
        #     total_loss += matcher_loss * self.opt.matcher_loss_alpha
            ...
            for frame_id in self.opt.frame_ids[1:]:
                ...
                for scale in self.opt.scales:
                    matcher_loss += compute_matcher_errors_from_correspondences(correspondences, outputs[("sample", frame_id, scale)],
                                                                                self.opt.width, self.opt.height, self.opt.batch_size, self.device, confidence=self.opt.confidence)
            total_loss += matcher_loss * self.opt.matcher_loss_alpha
            losses["loss/matcher"] = matcher_loss
    ...
    def save_opts(self):
        ...
        if not os.path.exists(models_dir):
            ...
            print(f"model path exist! {models_dir}")
        # else:
    ...
    def save_model(self):
        ...
        if self.epoch+1 == self.opt.num_epochs:
...
# 应该修改--model_name 为 fast/ab2_cuda0/c3vd_monodepth2_b8e30_me
'''
CUDA_VISIBLE_DEVICES=0 python train_corres.py --model_name fast/ab2_cuda0/c3vd_monodepth2_b8e30 --data_path rect_c3vd_data \
...
'''
CUDA_VISIBLE_DEVICES=1 python train_corres.py --model_name fast/ab2_cuda0/c3vd_monodepth2_b8e30_me_h_mla02 --data_path rect_c3vd_data \
  --dataset c3vdcorres --split c3vd --batch_size 8 --height 256 --width 320 --png --use_cudnn \
  --num_epochs 30 --scales 0 1 2 3 --num_workers=4 --scheduler_step_size 25 --half_epoch_matcher --matcher_loss_alpha 0.20
'''

'''
CUDA_VISIBLE_DEVICES=0 python train_corres.py --model_name fast/ab2_cuda0/c3vd_monodepth2_b8e30_raw --data_path rect_c3vd_data \
