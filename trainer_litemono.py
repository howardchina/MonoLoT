...
class Trainer:
    def __init__(self, options):
        ...
        # https://github.com/nianticlabs/monodepth2/issues/414
        # torch.backends.cudnn.enabled = False
        # # torch.backends.cudnn.benchmark = False
        # torch.backends.cudnn.deterministic = True
        # if self.opt.use_cudnn:
        ...
        self.num_pose_frames = 2 if self.opt.pose_model_input == "pairs" else self.frame_ids
        ...
        if not self.opt.disable_matcher:
            if self.opt.matcher_pretrained == "":
                self.opt.matcher_pretrained = "indoor"
            self.matcher = LoFTR(pretrained=self.opt.matcher_pretrained)
    ...
    def compute_losses(self, inputs, outputs):
        ...
        if not self.opt.disable_matcher:
            ...
            for frame_id in self.opt.frame_ids[1:]:
                ...
                for scale in self.opt.scales:
                    matcher_loss += compute_matcher_errors(correspondences, outputs[("sample", frame_id, scale)], self.opt.width, self.opt.height, self.opt.batch_size)
